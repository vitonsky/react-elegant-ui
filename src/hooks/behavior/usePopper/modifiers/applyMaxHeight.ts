...
function applyMaxHeightFn({
	state,
	options,
}: ModifierArguments<ApplyMaxHeightOptions>) {
	...
	const popupMaxHeight = userMaxHeight
		? Math.min(maxHeight - y, userMaxHeight)
		: maxHeight;
 ...
 */
export const applyMaxHeight: Modifier<
	'applyMaxHeight',
	ApplyMaxHeightOptions
> = {
	name: 'applyMaxHeight',
	enabled: true,
	fn: applyMaxHeightFn,
