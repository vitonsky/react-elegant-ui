					...
					{(contextRegistries) => {
						if (providedRegistriesRef.current === null) {
							...
							for (let i = 0; i < registries.length; i++) {
								...
								const overrides =
									providedRegistries[registry.id];
...
function withOverload<T>(
	overload: SimpleOverload<T>,
): IRegistryEntityOverload<T> {
	return {
...
function isOverload<T>(
	entity: IRegistryEntity<T>,
): entity is IRegistryEntityOverload<T> {
	return (
		(entity as IRegistryEntityOverload<T>).$symbol === registryOverloadMark
	);
}
...
export class Registry {
	...
	merge(otherRegistry?: Registry) {
		...
		const otherRegistryEntities =
			otherRegistry.snapshot<IRegistryEntities>();
	...
	private mergeEntities(
		...
		if (isOverload(overrides)) {
			...
			if (isOverload(base)) {
				...
				return withOverload((Base) =>
					overrides.overload(base.overload(Base)),
				);
			}
